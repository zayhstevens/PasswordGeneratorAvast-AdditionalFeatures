...
import { encryptSensitive, apiSaveRecord } from "./vault.js";
...
async function exportCsv(kind){
  const filename = kind === "plain" ? "vault_plain.csv" : "vault_encrypted.csv";
  const candidates = kind === "plain"
    ? ["/api/vault/plain", "./data/vault_plain.csv"]
    : ["/api/vault/encrypted", "./data/vault_encrypted.csv"];
  ...
  if (text == null) throw new Error("Export failed");
...
function validateCard({ number, cvc, exp }) {
  const num = (number || "").replace(/\s+/g, "");
  const anyProvided = Boolean(num || (cvc || "").trim() || (exp || "").trim());

  // Entire card section optional
  ...
  if (!exp || !exp.trim()) errors.exp = "Expiration is required if adding a card.";

  const cvcTrim = (cvc || "").trim();
  if (!cvcTrim) errors.cvc = "CVC is required if adding a card.";
  else if (!/^\d+$/.test(cvcTrim)) errors.cvc = "Digits only.";
  else {
