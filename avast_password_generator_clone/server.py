...
ENC_HEADER = ["website", "username", "encPayload", "iv", "createdAt", "cardType"]
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/vault/plain":
            self._text(200, PLAIN_PATH.read_text(encoding="utf-8"), "text/csv; charset=utf-8")
            return
        if parsed.path == "/api/vault/encrypted":
            self._text(200, ENC_PATH.read_text(encoding="utf-8"), "text/csv; charset=utf-8")
            return
    ...
    def _handle_save(self, data: dict) -> None:
        ...
        enc_payload = (data.get("encPayload") or "").strip()
