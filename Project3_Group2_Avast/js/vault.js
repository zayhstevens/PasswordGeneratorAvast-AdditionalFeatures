...
export function buildPlainVaultCsv(records) {
  const header = ["website","username","password","cardNumber","cardName","cardExp","cardCvc","cardZip","cardType","createdAt","projectStrength","avastStrength"];
...
export function buildEncryptedVaultCsv(records) {
  const header = ["website","username","encPassword","ivPassword","encCard","ivCard","createdAt"];
  const lines = [header.join(",")];
  for (const rec of records) {
    const row = header.map((k) => csvEscape(rec[k]));
    lines.push(row.join(","));
...
// Encrypt just the password string
export async function encryptPassword(password) {
  const key = await getOrCreateVaultKey();
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const data = new TextEncoder().encode(String(password ?? ""));
  const ct = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, key, data);
  return { encPassword: b64(ct), ivPassword: b64(iv) };
}

// Encrypt card fields as a JSON object (returns empty strings if no card data)
export async function encryptCard(cardObj) {
  const { cardNumber, cardName, cardExp, cardCvc, cardZip } = cardObj;
  const hasCard = Boolean((cardNumber || "").trim() || (cardName || "").trim() || (cardExp || "").trim() || (cardCvc || "").trim() || (cardZip || "").trim());
  if (!hasCard) return { encCard: "", ivCard: "" };

  const key = await getOrCreateVaultKey();
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const data = new TextEncoder().encode(JSON.stringify({ cardNumber, cardName, cardExp, cardCvc, cardZip }));
  const ct = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, key, data);
  return { encCard: b64(ct), ivCard: b64(iv) };
}

// Kept for any legacy callers — encrypts everything together
...
// Legacy combined decrypt (kept for compatibility)
...
// Decrypt just the password (returns a plain string)
export async function decryptPassword(encPassword, ivPassword) {
  const key = await getOrCreateVaultKey();
  const ct = unb64(encPassword);
  const ivBytes = unb64(ivPassword);
  const pt = await crypto.subtle.decrypt({ name: "AES-GCM", iv: ivBytes }, key, ct);
  return new TextDecoder().decode(pt);
}

// Decrypt card fields (returns { cardNumber, cardExp, cardCvc, cardZip } or null if empty)
export async function decryptCard(encCard, ivCard) {
  if (!encCard || !ivCard) return null;
  const key = await getOrCreateVaultKey();
  const ct = unb64(encCard);
  const ivBytes = unb64(ivCard);
  const pt = await crypto.subtle.decrypt({ name: "AES-GCM", iv: ivBytes }, key, ct);
  return JSON.parse(new TextDecoder().decode(pt));
}
